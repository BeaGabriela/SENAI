...
select Telefone from Telefones where cliente_id = 12; --select * from telefones where cliente_id = (select cliente_id from clientes where nome= 'Cesar Augustos Pascali Rago');
select count(*) as 'Telefone' from Telefones where cliente_id=12; 
delete from Telefones where cliente_id=12;
insert into value(12, 19991865503 ) Telefones where cliente_id=12;
...
insert into clientes value (default, 'Joaquim Inácio Silva', 'Rua Malaga', '22','Contagem', 'Santa Cruz Industrial', 'hospital');
insert into Telefones (Cliente_id, Telefone) values (106, 19989995511 );
--Pizza Baiana
insert into Pedidos value(default, 106, curdate(), curtime(),  32.13);
insert into Itens_Pedido value(27, 6, 1, 32.13);
--Pizza Atum
insert into Pedidos value(default, 106, curdate(), curtime(),  32.29);
...
--Quantas pizzas ele pediu e
 ...
 --Crie uma view da tabela Itens_pedido que mostre também o nome de cada pizza em ordem de pedido_id, coloque o nome de "vw_itens"
  ...
  select i.pedido_id, i.pizza_id, i.quantidade, i.valor, p.nome as Pizzas 
...
select p.pedido_id, p.cliente_id, p.data, p.hora, pz.pizza_id, pz.nome,i.quantidade, pz.valor, i.valor * i.quantidade as 'SubTotal', p.valor as 'Total' 
from Itens_Pedido i inner join Pizzas pz
...
create view vw_pedidos as 
select p.pedido_id, p.cliente_id, c.nome as 'Nome cliente', p.data, p.hora, pz.pizza_id, pz.nome,i.quantidade, pz.valor, i.valor * i.quantidade as 'SubTotal', p.valor as 'Total' 
from Itens_Pedido i inner join Pizzas pz
on i.pizza_id = pz.pizza_id
inner join Pedidos p on i.pedido_id = p.pedido_id;
